...
function Pessoas({ pessoas, recarregar, setErro }) {
  ...
  async function remover(pessoa) { if (!confirm(`Excluir ${pessoa.nome}?`)) return; setErro(''); try { await pessoasApi.excluir(pessoa.id); await recarregar(); } catch (e) { setErro(e.message); } }
